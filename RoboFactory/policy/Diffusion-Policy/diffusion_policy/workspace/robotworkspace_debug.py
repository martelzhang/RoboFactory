...
class RobotWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        debug_dir = os.path.join(self.output_dir, 'debug')    # 1. action 2. log for loss
        saved_action_dir = os.path.join(debug_dir, 'action')
        os.makedirs(saved_action_dir, exist_ok=True)
        # load checkpoint
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        loss, pred = self.model.compute_loss(batch, output_pred=True)
                        for i in range(B):
                            task_id = B * batch_idx + i                            
                            torch.save(pred[i], os.path.join(saved_action_dir, f'{task_id}.pt'))
                        with open(os.path.join(debug_dir, f'task_loss.log'), 'a') as f:
